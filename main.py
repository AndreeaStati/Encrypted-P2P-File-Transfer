...
if __name__ == "__main__":
    ...
    for i in range(0, 9):
        data = encrypt(data)
        p[x] = (data >> 32) & 0xffffffff
        p[x+1] = data & 0xffffffff
        x += 2

    input_str = input("Enter data to encrypt (max 8 chars): ")
    input_int = str_to_int(input_str)
    
    if input_int.bit_length() <= 64:
        data_encrypted = encrypt(input_int)
        print(f"Encrypted (Hex): {hex(data_encrypted)}")
        
        data_decrypted = decrypt(data_encrypted)
        
        try:
            output_str = int_to_str(data_decrypted)
            print(f"Decrypted text: {output_str}")
        except UnicodeDecodeError:
            print("Eroare: Decriptarea nu a produs text valid. Verifica cheia!")
