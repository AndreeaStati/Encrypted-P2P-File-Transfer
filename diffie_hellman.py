...
def generate_keypair():
    """Genereaza o pereche de chei (privata, publica) pentru o statie."""
    private_key = random.getrandbits(256)
    public_key = pow(DH_G, private_key, DH_P)
    return private_key, public_key

def calculate_blowfish_key(my_private_key, other_public_key):
    """Calculeaza secretul comun si il transforma intr-o cheie de bytes pentru Blowfish."""
    shared_secret = pow(other_public_key, my_private_key, DH_P)
    
    # Transform nr urias in bytes (maxim 56 bytes pt Blowfish)
    secret_bytes = shared_secret.to_bytes((shared_secret.bit_length() + 7) // 8, byteorder='big')
    
    # Blowfish suportă chei intre 4 - 56 bytes
    return secret_bytes[:56]